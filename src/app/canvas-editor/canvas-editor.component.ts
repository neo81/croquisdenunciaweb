...
// Componente responsable exclusivamente del canvas Konva.
// Recibe la URL del fondo vía @Input y emite el JSON serializado.
// ============================================================
...
export class CanvasEditorComponent implements AfterViewInit, OnDestroy, OnChanges {
  ...
  // ── Inputs / Outputs ────────────────────────────────────────
  /** Nombre del archivo de fondo seleccionado (ej: 'foto1.jpg'). Vacío = sin fondo. */
  @Input() backgroundFile: string = '';

  /** Color de trazo activo para dibujar y para texto */
  @Input() strokeColor: string = '#000000';

  /** JSON serializado del croquis (two-way via ngModel-like pattern) */
  ...
  ngOnChanges(changes: SimpleChanges): void {
    // Reaccionar al cambio de fondo desde el componente padre
  ...
  /** Transforma posición del puntero en coordenadas lógicas del canvas */
  ...
  // ── Herramientas públicas (llamadas desde la toolbar) ────────
  ...
  public flipHorizontal(): void {
    if (this.selectedNode) {
      this.selectedNode.scaleX(this.selectedNode.scaleX() * -1);
      this.layer.draw();
    }
  }
  ...
  public flipVertical(): void {
    if (this.selectedNode) {
      this.selectedNode.scaleY(this.selectedNode.scaleY() * -1);
      this.layer.draw();
    }
  }
  ...
  public duplicateSelected(): void {
    ...
    const clone = this.selectedNode.clone({
      x: this.selectedNode.x() + 20,
      y: this.selectedNode.y() + 20,
    });
  ...
  public saveToJson(): void {
    ...
    const json = JSON.stringify(stageObj, null, 2);
    this.savedJsonChange.emit(json);
  }
  ...
  public onHtmlDragStart(evt: DragEvent, iconFile: string): void {
    evt.dataTransfer?.setData('text/plain', iconFile);
  }
  ...
  private handleMouseMove(_e: any): void {
    ...
    const logicPos = this.getLogicalPosition();
    this.lastLine.points(this.lastLine.points().concat([logicPos.x, logicPos.y]));
